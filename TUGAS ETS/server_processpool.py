...
def handle_client(connfd, addr):
    with socket.socket(fileno=connfd) as conn:
...
def main():
    parser = argparse.ArgumentParser(description="ProcessPool Server")
    parser.add_argument("--workers", type=int, default=50,
                        help="Jumlah process worker (default: 50)")
    args = parser.parse_args()

    print(f"Starting ProcessPool server with {args.workers} workers...")
    try:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while True:
            ...
            p = Process(target=handle_client, args=(conn.fileno(), addr))
            p.daemon = True
            p.start()
            conn.close()  # Penting: parent harus close, agar ownership ke child
